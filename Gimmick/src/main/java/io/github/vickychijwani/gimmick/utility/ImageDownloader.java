...
public class ImageDownloader {
    ...
    private static Bitmap getBitmapFromDisk(String urlString, File imageFile) {
        if (DeviceUtils.isExtStorageAvailable()) {
            if (imageFile.exists()) {
                // disk cache hit
    ...
    private static Bitmap downloadBitmap(String urlString, boolean isDiskCaching, File imageFile) {
        try {
            InputStream inputStream = NetworkUtils.downloadUrl(urlString);
            ...
            try {
                ...
                Bitmap bitmap;
                if (isDiskCaching && DeviceUtils.isExtStorageAvailable()) {
                    FileOutputStream outputstream = new FileOutputStream(imageFile);
                    try {
                        DeviceUtils.copy(new FlushedInputStream(inputStream), outputstream);
                    } finally {
                        outputstream.close();
                    }
